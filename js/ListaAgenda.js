...
function excluirEventoDaListEnviarLembrete(e) {
    ...
    var splitCookieEvento = docCookies.getItem('CookieListaAgendaSaneago').split(',');
    
    splitCookieEvento = $.grep(splitCookieEvento, function (value) {
        return value != excluirId;        
    });
    docCookies.setItem('CookieListaAgendaSaneago', splitCookieEvento);
        
    var splitCookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaSaneago").split(',');
    ...
    for (var i = 0; i < splitCookieLembrete.length; i++) {
        if (splitCookieLembrete[i].split('&D')[0] == excluirId) {
            ...
            docCookies.setItem('CookieEnviarLembreteAgendaSaneago', splitCookieLembrete);
        }
    }
...
function addEventoAListEnviarLembrete(e) {
    ...
    if (e.currentTarget.checked == true) {
        ...
        var cookieEvento = docCookies.getItem('CookieListaAgendaSaneago');
        ...
        if (typeof cookieEvento != 'undefined' && cookieEvento != "" && cookieEvento != null) { // Se cookie já tiver valor
            ...
            if ($.inArray(idEvento, splitCookieEvento) == -1) // Se idEvento NÃO estiver no cookie
            {
                ...
                var cookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaSaneago");  //  Adiciona o cookie
                ...
                docCookies.setItem("CookieEnviarLembreteAgendaSaneago", splitCookieLembrete);  //  evento
            ...
            docCookies.setItem("CookieListaAgendaSaneago", idEvento);
            docCookies.setItem("CookieEnviarLembreteAgendaSaneago", idEvento + '&D' + diasParaLembrete);
...
function MarcarLembrete() {
    ...
    var getCookieLembrete = docCookies.getItem('CookieEnviarLembreteAgendaSaneago').split(',');
